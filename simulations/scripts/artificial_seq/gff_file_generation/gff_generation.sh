...
#SBATCH --output=/*.out #specify a file for bash output to be held 
#SBATCH --error=/*.err #specify a file for bash error to be held
#SBATCH --job-name=* specify a job name if running on SLURM
#SBATCH -c 4 #uses 4 cores, can change depending on your system's requirements
#SBATCH --mem=10G #uses 10G total, can change depending on your system's requirements
...
#replace the variables in the general definitions section at the top of the page, as well as the SLURM variables
#outlined above if you are running on a shared HPC system with SLURM.
#You will need to also change your file paths, as of 12/12/2024. The idea is to update the script to have variables
...
#createFakeSequence.pl run. To be edited into a workflow where these parameters will not need to be changed every run.
...
#if you chose to generated 10 sequences of length 500Mb from a particual species model you will need to put END=9.
#PRIOR TO RUNNING THIS SCRIPT YOU MUST SPLIT THE GARLIC OUTPUT FASTA FILES ACCORDINGLY. THE SINGLE ERROR FILE
#GENERATED BY RUNNING GARLIC IS FINE AS IS.

###Running on one generated sequence:
#In the case that you are only working with one sequence, make sure END=0 and replace all 'for num in $(seq 1 ${END})'
# or 'for num in $(seq 1 ${LAST})' with 'for num in '$(seq ${LAST} ${LAST})'. Also, be sure to comment out section 1.


END=* #total number of sequences generated with Garlic run, minus 1
model=* #model used for fake fastas
export model #sets model variable for later on awk script
out_header=* #header of the output log file
log_file=/*.err #error file path generated by GARLIC
LAST=$((${END}+1)) #total number of sequences generated

#Other lines below that need to be edited to mirror researcher's nomenclature: 78 (output_file), 83 (output_file_2),
#98 (output_file_3), 100 (output_file_4), 124 (search_file), 125 (nest_file), 133 (final_file), 177 (output_file),
#182 (output_file_2), 197 (output_file_3), 199 (output_file_4), 223 (search_file), 224 (nest_file), 232 (final_file),
#261-270 (intermediate file folders), 273-277 (paths to files), 281 (directory), 303 (gff directory), 308 (sing_file),
...
for num in $(seq ${LAST} ${LAST}); do #for numbers from 1 to the second to last sequence, do ***EDIT BEFORE RUNNING ON WHOLE SERIES to be 1 ${END}
	...
	output_file=/*/${model}_log_${num}.txt #define the output file based on the model and artificial sequence number
	...
	output_file_2=/*/${model}_inserts_list_${num}.txt #define the output for the inserts of the num artificial sequence based on the log file
...
###################################################################################################################################################
	...
	output_file_3=/*/${model}_inserts_nest_${num}.txt #define the output file which only includes the nested inserts 
	...
	output_file_4=/*/${model}_inserts_sing_${num}.txt #define the output file which only includes non-nested, single inserts
...
###################################################################################################################################################
	...
	search_file=/*/${model}_${num}_inserts.txt #define the search file (the file containing all of the inserts and their sequences, generated by garlic itself and not the log file)
	nest_file=/*/${model}_int_nest_${num}_file #define an intermediate nested sequence file
	echo "Nested file: ${nest_file}"
        ...
        final_file=/*/final_${model}_${num}_nested.txt #define the final nested insert file
...
mv /*/${model}*log* ${log_dir} #move final split log files to the log directory
mv /*/${model}*sing* ${sing_dir} #move final single TE files to the single directory
mv /*/${model}*nest_* ${nest_dir} #remove if works well
mv /*/${model}*list* ${list_dir} #move the combined insert files to the list directory
mv /*/final*${model}* ${final_dir} #move final nested files to the nest directory
...
mkdir -p /*/${model} #make a directory for gff files

for num in $(seq ${LAST} ${LAST}); do
    echo "Sequence number: ${model}_artificial_sequence_${num}" #define the sequence number

    sing_file=/*/${model}_inserts_sing_${num}.txt #define the file with single TE inserts
	...
	for value in ${start_pos[@]}; do #for every starting position
                ...
                echo -e "${model}_artificial_sequence_${num}\tGARLIC\t${repeat}\t${start}\t${stop}\t100\t${strand}\t.\t${extra1};${extra2}" \
                        >> /*/${model}_artificial_sequence_${num}.gff #build the gff file
    done
	cat /*/${model}_artificial_sequence_${num}.gff | sort --unique > /*/${model}_artificial_sequence_${num}_un.gff

	rm /*/${model}_artificial_sequence_${num}.gff
	mv /*/${model}_artificial_sequence_${num}_un.gff /*/${model}/${model}_artificial_sequence_${num}.gff
    ...
    length=$(cat /*/${model}_artificial_sequence_${num}.gff | wc -l)
    match=$(cat ${sing_file} | wc -l)
    echo "File can be found at /*/${model}_artificial_sequence_${num}.gff"
...
for num in $(seq ${LAST} ${LAST}); do
	...
	final_nest_file=/*/final_${model}_${num}_nested.txt #defines the final nested file
	search_term2=$(cat ${final_nest_file} | cut -f 2) #builds the search term file based on the stop position of each nested TE
	mkdir -p /*/${model}/${num} #make directory to house intermediate ncbi files
	dir=/*/${model}/${num} #define the directory path
	for stop in ${search_term2[@]}; do #for every stop position of a nested TE
		...
		rm /*/${model}/${num}/insert_${stop}_refx.fasta #remove intermediate file
		cd ${dir}
		...
		cat ${align_file} | sort -nrk4 | awk -f /*/sequence_start_stop.awk | awk '!seen[$2]++' | awk '!seen[$3]++' > ${dir}/${stop}.out #sort the alignment file from largest element to shortest element, keep only 100% matches, remove duplicated matches or overlap from similar elements, ie. keep the longest
...
#print the insert names from the out file, #print the starting position numbers, #print the ending position numbers, #print the strand types, + or -, #print the extra1 information, *note no extra 2 for these, could see about adding later, & #print the repeat names
		...
		awk -v num="${num}" -v model="${model}" -f /*/nested_gff.awk ${dir}/${stop}.int3 \
		...
		cat ${dir}/${stop}.final >> /*/${model}/${model}_artificial_sequence_${num}_nest.gff #put file contents into new gff
	    ...
	    echo -e "${model}_artificial_sequence_${num}\tGARLIC\t${repeat_2}\t${actual_start}\t${stop_ini}\t100\t${strand_2}\t.\t${extra1_2};${extra2_2}" \
        >> /*/${model}/${model}_artificial_sequence_${num}_nest.gff


	done
	count_4=$(cat /*/${model}/${model}_artificial_sequence_${num}_nest.gff | wc -l) #count the full file length after adding in nested TEs
	...
	mkdir -p /*/${model}/${num}/alignments #make an alignment directory
	align_dir=/*/${model}/${num}/alignments #define the directory
	mkdir -p /*/${model}/${num}/final
	final_dir=/*/${model}/${num}/final
	mkdir -p /*/${model}/${num}/int
	int_dir=/*/${model}/${num}/int
	mv /*/${model}/${num}/*align ${align_dir} #move the alignment files to the alignment directory
	mv /*/${model}/${num}/*int* ${int_dir} #move intermediate files to alignment directory
	mv /*/${model}/${num}/*out ${int_dir} #move the temporary output files to the alignment directory
	mv /*/${model}/${num}/*final ${final_dir} #move the final files to the final directory
	...
	cat /*/${model}/${model}_artificial_sequence_${num}_nest.gff /*/${model}/${model}_artificial_sequence_${num}.gff \
	>> /*/${model}/${model}_artificial_sequence_${num}_int.gff
	sort -k 4n /*/${model}/${model}_artificial_sequence_${num}_int.gff > /*/${model}/${model}_artificial_sequence_${num}_final.gff
	rm /*/${model}/${model}_artificial_sequence_${num}_int.gff
done
