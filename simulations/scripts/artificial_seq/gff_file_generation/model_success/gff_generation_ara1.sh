...
#SBATCH --output=/datacommons/yoderlab/programs/Garlic-1.4/Garlic-1.4/log/scripts/gff_gen_outputs/arabidop_test2.out
#SBATCH --error=/datacommons/yoderlab/programs/Garlic-1.4/Garlic-1.4/log/scripts/gff_gen_outputs/arabidop_test2.err
#SBATCH --partition=yoderlab
#SBATCH --account=yoderlab
#SBATCH --mail-type=ALL
#SBATCH --mail-user=hpk4@duke.edu
#SBATCH --job-name=ara1_test
#SBATCH -c 4
#SBATCH --mem=10G
...
END=0 #total number of sequences generated with Garlic run, minus 1
model=ara1 #model used for fake fastas
export model #sets model variable for later on awk script
out_header=ara1_100Mb_fin #header of the output log file, based on DCC organization
...
final_out_dir=/datacommons/yoderlab/users/hkania/garlic_gffs/${model}
...
for num in $(seq ${LAST} ${LAST}); do
	...
	for stop in ${search_term2[@]}; do #for every stop position of a nested TE
		...
		cat ${align_file} | sort -nrk4 | awk -f /datacommons/yoderlab/programs/Garlic-1.4/Garlic-1.4/log/scripts/sequence_start_stop.awk | awk '!seen[$2]++' | awk '!seen[$3]++' > ${dir}/${stop}.out #sort the alignment file from largest element to shortest element, keep only 100% matches, remove duplicated matches or overlap from similar elements, ie. keep the longest
...
#print the insert names from the out file, #print the starting position numbers, #print the ending position numbers, #print the strand types, + or -, #print the extra1 information, *note no extra 2 for these, could see about adding later, & #print the repeat names
	...
	echo "Cleaning up gff files, AT THE MOMENT KEEPING MOST INTERMEDIATE FILES DEC 2024"
        cat /datacommons/yoderlab/users/hkania/garlic_gffs/${model}/${model}_artificial_sequence_${num}_nest.gff /datacommons/yoderlab/users/hkania/garlic_gffs/${model}/${model}_artificial_sequence_${num}.gff \
        /datacommons/yoderlab/users/hkania/garlic_gffs/${model}/${model}_artificial_sequence_${num}_test.gff
        sort -k 4n /datacommons/yoderlab/users/hkania/garlic_gffs/${model}/${model}_artificial_sequence_${num}_test.gff > /datacommons/yoderlab/users/hkania/garlic_gffs/${model}/${model}_artificial_sequence_${num}_final.gff
        rm /datacommons/yoderlab/users/hkania/garlic_gffs/${model}/${model}_artificial_sequence_${num}_test.gff


done
echo "Finished Nested TE GFF Generation for all specified sequences"
echo "Model ${model}, first artifical sequence: ${END}, last artificial sequence: ${LAST}"
echo "Final GFF files can be found in ${final_out_dir}"
date
