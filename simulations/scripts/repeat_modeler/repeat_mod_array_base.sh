...
#SBATCH --array=1-4%4 #run an array of length 2, with max 2 array tasks at once, can change
#SBATCH --mem=2G #2G of memory per array task
...
## This script runs Repeat Modeler over a set of 'base' fastas, meaning they are
## not supposed to have any repeat content. The output is typical Repeat Modeler files.
